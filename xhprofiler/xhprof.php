 ...
 *
 * Which prefix is meant? The Namespace?
 *
 * SQL copied from \xhprof\xhprof_lib\Db\Pdo.php
 */
...
// Create Database
$pdo->exec("CREATE DATABASE IF NOT EXISTS `" . $_xhprof['dbname'] . "` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci; USE `" . $_xhprof['dbname'] . "`;'");

// create details table, if it does not exist, yet
// this is run on initial setup
if (false === tableExists($pdo, 'details')) {
    try {
        $pdo->exec(
            $sql .
            "CREATE USER '".$_xhprof['dbuser']."'@'localhost' IDENTIFIED BY '".$_xhprof['dbpass']."';
            GRANT ALL ON `".$_xhprof['dbname']."`.* TO '".$_xhprof['dbuser']."'@'localhost';
            FLUSH PRIVILEGES;"
...
function tableExists($pdo, $table) {
    ...
    // table found ($result is a PDOStatement Object)
    return true;
}

unset($pdo);

/* End of XHProf Setup Script */
/*
